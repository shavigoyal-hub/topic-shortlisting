...
var FORMAT_RX = /\b(login|sign in|apk|download|coupon|promo code|discount code|cracked|torrent|free pdf)\b/;
...
function ensureAllSheets(){
  ...
  var DEFAULTS = [['offering','Both'],['website',''],['category',''],['services',''],['products',''],['industries',''],
...
function getConfig(){
  ...
  return { offering:o.offering||'Both', website:o.website||'', category:String(o.category||'').trim(), services:list('services'), industries:list('industries'), products:list('products'),
...
function siteCheck_(dom, items, gl){   // items: [{kw, q}] — q is the core product term to search (LLM-extracted, word-list fallback)
  var key=prop('SERPER_KEY'); if(!key || !dom || !items.length) return {};
  var reqs=items.map(function(it){ return { url:'https://google.serper.dev/search', method:'post', headers:{'X-API-KEY':key}, contentType:'application/json', muteHttpExceptions:true, payload:JSON.stringify({q:'site:'+dom+' '+(it.q||it.kw), gl:gl||'us', num:10}) }; });
  var genuineByKw={}, haveAny=[];
  for(var i=0;i<reqs.length;i+=20){
    var resp=UrlFetchApp.fetchAll(reqs.slice(i,i+20));
    for(var j=0;j<resp.length;j++){ var kw=items[i+j].kw, genuine=[];
...
/* ---- GUARDS (ported from bulk_audit.mjs) — deterministic vetoes so the AI can't reject a keyword that IS
   the client's own category/offering. inCategory: 5-char-prefix match on distinctive category words (protects even
   under SERP). inOffering: a meaningful keyword term appears in the offering list. Both err toward keep. ---- */
var GUARD_GENERIC={the:1,and:1,'for':1,'with':1,your:1,you:1,are:1,can:1,how:1,what:1,why:1,does:1,from:1,that:1,'this':1,into:1,best:1,top:1,near:1,free:1,cost:1,price:1,guide:1,tips:1,ideas:1,service:1,services:1,product:1,products:1,solution:1,solutions:1,company:1,companies:1,custom:1,professional:1,online:1,list:1,types:1,type:1,about:1,more:1,other:1,their:1,they:1,when:1,where:1,which:1,will:1,make:1,made:1,need:1,using:1,used:1,vs:1};
var GUARD_CATSTOP={services:1,service:1,solutions:1,solution:1,company:1,companies:1,group:1,agency:1,firm:1,and:1,the:1,'for':1,'with':1,your:1,business:1,industry:1,provider:1,providers:1,professional:1,training:1,'trainings':1,course:1,courses:1,coaching:1,coach:1,skill:1,skills:1,customer:1,customers:1,support:1,supporting:1,team:1,teams:1,staff:1,employee:1,employees:1,online:1,virtual:1,corporate:1,program:1,programs:1,management:1,consulting:1,consultant:1,consultants:1,marketing:1,digital:1,software:1,technology:1,technologies:1,systems:1,system:1,media:1,design:1,studio:1,learning:1,education:1,academy:1,platform:1,tools:1,products:1,product:1};
function guardStem(t){ return t.replace(/ies$/,'y').replace(/s$/,''); }
function inOffering(kw, names){
  if(!names || !names.length) return false;
  var hayToks=names.join(' ').toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ').filter(String).map(guardStem);
  var hay={}; hayToks.forEach(function(t){hay[t]=1;}); var hayStr=' '+hayToks.join(' ')+' ';
  var toks=String(kw||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ').filter(function(t){return t&&t.length>=4&&!GUARD_GENERIC[t];}).map(guardStem);
  for(var i=0;i<toks.length-1;i++){ if(hayStr.indexOf(' '+toks[i]+' '+toks[i+1]+' ')>=0) return true; }   // phrase match
  return toks.some(function(t){ return t.length>=4 && hay[t]; });
}
function inCategory(kw, category){
  if(!category) return false;
  var catToks=String(category).toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ').filter(function(t){return t.length>=5&&!GUARD_CATSTOP[t];}).map(function(t){return t.slice(0,5);});
  if(!catToks.length) return false;
  var set={}; catToks.forEach(function(t){set[t]=1;});
  var kwToks=String(kw||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ').filter(function(t){return t.length>=5;}).map(function(t){return t.slice(0,5);});
  return kwToks.some(function(t){ return set[t]; });
}
...
function classifyBatch(items, cfg){
  ...
  (j.results||[]).forEach(function(o){ byId[String(o.id)]={ audience:AUDIENCES.indexOf(o.audience)>=0?o.audience:'General', type:(o.type||'').toString().slice(0,40), keep:o.keep!==false, reason:o.keep!==false?'':(REJECT_REASONS.indexOf(o.reason)>=0?o.reason:'No commercial intent'), conf:(function(x){x=String(o.confidence||'').toLowerCase();return (x==='high'||x==='medium'||x==='low')?x:'low';})(), explain:(o.explain||'').toString().slice(0,160), profession:(o.profession||'').toString().slice(0,40),
    services:(Object.prototype.toString.call(o.services)==='[object Array]'?o.services:[]).map(function(s){return String(s).trim();}).filter(String).slice(0,4), icp:(o.icp||'').toString().slice(0,50), icpFit:o.icpFit!==false, product:(o.product||'').toString().slice(0,60) }; });
  return byId;
}
...
function processBatch(phase){
  ...
  todo.forEach(function(i){ var v=vals[i], kw=String(v[COL.KW-1]).toLowerCase(), c=cache[kw];
    ...
    if(v[COL.STATUS-1]!=='1' && String(v[COL.LAYER-1])!=='Locked'){   // human keeps (1) and locked overrides are protected; review the rest manually
      ...
      // GUARD: a distinctive word from the client's own BUSINESS CATEGORY makes it their field by definition → never AI-reject.
      // Matches bulk under-SERP behavior: only inCategory applies (inOffering is too loose — "kitchen" in "Kitchen Cabinets"
      // would wrongly protect "commercial kitchen design" — so it yields to SERP, which this tool always runs).
      var guarded=inCategory(v[COL.KW-1], cfg.category);
      if(hit){ v[COL.STATUS-1]='0'; v[COL.REASON-1]=hit.reason; v[COL.REXP-1]=hit.reason; v[COL.LAYER-1]='Rule'; }                       // rules are definitive
      else if(c.keep===false && guarded){ v[COL.STATUS-1]='1'; v[COL.REASON-1]='In-field (client\'s own category/offering)'; v[COL.REXP-1]=v[COL.REASON-1]; v[COL.LAYER-1]='Guard'; }   // guard veto: AI wanted to reject the client's own field
  ...
  if(dom && prop('SERPER_KEY')){
    var rej=[], need=[];
    todo.forEach(function(i){ var v=vals[i]; if(String(v[COL.LAYER-1])==='AI' && String(v[COL.STATUS-1])==='0'){ var lk=String(v[COL.KW-1]).toLowerCase(); rej.push({i:i, kw:v[COL.KW-1], lk:lk}); if(!cache[lk] || cache[lk].siteChecked!==true) need.push({kw:v[COL.KW-1], q:((cache[lk]&&cache[lk].product)||coreQuery_(v[COL.KW-1]))}); } });
    if(need.length){ var found=siteCheck_(dom, need, cfg.serpGl); need.forEach(function(it){ var lk=String(it.kw).toLowerCase(); if(!cache[lk]) cache[lk]={}; cache[lk].siteChecked=true; cache[lk].siteKeepUrl=found[lk]||''; }); }
