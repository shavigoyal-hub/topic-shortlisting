...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const rows = dec.map(d => {
      ...
      return { id: d.it.id, status: d.status, confidence: d.status === '' ? '' : c.conf, reason: d.reason, explained, audience: c.audience || '', profession: c.profession || '', type: c.type || '', modifier: modifiersOf(d.it.kw, cfg), bofu: isBofu(d.it.kw) ? 'Yes' : 'No', services: (c.services || []).join(', '), buyStage: c.buyStage || '', icp: c.icp || '', icpFit: c.icpFit === false ? 'no' : 'yes', titles: Array.isArray(d.it.titles) ? d.it.titles : [] };
    });
    res.statusCode = 200;
